...
def main():
    ...
    if opts["daemon"]:
        # daemon migrado para wallpha-plasma (canônico). Delega se instalado, senão fallback local.
        import os
        from pathlib import Path

        plasma_cands = [
            Path.home() / "dev/wallpha/wallpha-plasma/bin/wallpha-plasma-daemon",
            Path.home() / ".local/bin/wallpha-plasma-daemon",
            Path("/usr/local/bin/wallpha-plasma-daemon"),
            Path("/usr/bin/wallpha-plasma-daemon"),
        ]
        for cand in plasma_cands:
            if cand.is_file() and os.access(cand, os.X_OK):
                os.execv(str(cand), [str(cand)] + sys.argv[1:])
        # fallback: roda daemon local (legado) se wallpha-plasma não instalado
        daemon_run()
        return
