...
def _warn_updates():
    try:
        ...
        if not local:
            return
        # verifica wallpha-plasma sempre
        latest_plasma = _latest_tag("wallpha-plasma")
        if latest_plasma:
            # tenta ler plasma local se estivermos no cli
            plasma_local = None
            if not is_plasma:
                ...
                if plasma_local is None:
                    # fallback dev plasma ou local
                    plasma_local = latest_plasma  # não avisa se não achar
